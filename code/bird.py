    ...
    def get_inputs(self,pipes):
        ...
        for p in pipes:
            if p.pipe_type == PIPE_UPPER and p.rect.right < closest and p.rect.right > self.rect.left:
                ...
                bottomy = p.rect.bottom
        horizontal_distance = closest - self.rect.centerx
        vertical_distance = (self.rect.centery) - (bottomy + PIPE_GAP_SIZE /2)
        inputs = [
            ((horizontal_distance/DISPLAY_W)*0.99) + 0.01, (((vertical_distance + Y_SHIFT)*0.99)+0.01)
        ]
        return inputs
...
class BirdCollection():
    ...
    def __init__(self, gameDispaly):
        self.gameDisplay = gameDispaly
        self.birds = []
    ...
    def evolve_population(self):
        ...
        cutoff = int(len(self.birds) * MUTATION_CUT_OFF)
        good_birds = self.birds[0:cutoff]
        bad_birds = self.birds[cutoff:]
